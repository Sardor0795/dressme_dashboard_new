  ...
  return (
    <div className="w-full h-fit border border-red-500">
      <section className="w-full flex flex-col flex-wrap h-fit gap-x-[10px] ">
        <article className=" h-[650px] w-full overflow-hidden">
          <Slider
          ...
          >
            {imgGroup?.map((data) => {
              return (
                <article key={data?.id}>
                  {/* <figure className=" w-full h-full overflow-hidden border border-searchBgColor bg-btnBgColor rounded-lg flex items-center justify-center"> */}
        ...
        <article className="flex flex-row  w-full mt-4 border border-red-500">
          <Slider
            asNavFor={nav1}
            ref={slider2}
            // slidesToShow={5}
