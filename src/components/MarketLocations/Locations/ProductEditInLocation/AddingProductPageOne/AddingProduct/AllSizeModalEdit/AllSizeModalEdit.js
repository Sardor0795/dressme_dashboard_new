...
function AllSizeModalEdit({ onClick, modalOpenColor, colorGroup }) {
  ...
  return (
    <div className="w-[670px] h-fit bg-white rounded-lg bg-white py-5 px-4 border border-red-500">
      ...
      <div className="flex items-center pl-7 mt-2">
        <div className="w-fit flex items-center gap-x-2">
          <span className="text-black text-sm not-italic font-AeonikProRegular">Выбранный цвет:</span>
          {colorGroup.map((data) => {
            return (
              <>
                {data?.action && (
      ...
      {/* All Cards */}
      <div className="h-[694px] overflow-auto flex flex-col gap-y-2 mt-6 AllSizeModalScroll">
        {/* Cards */}
                  ...
                  {colorGroup.map((data) => {
                    return (
                      <>
                        {data?.action && (
                          <button
                            className={`h-[22px] rounded-[15px] text-white px-3  flex items-center justify-center text-sm not-italic font-AeonikProRegular bg-${data?.colors}`}
